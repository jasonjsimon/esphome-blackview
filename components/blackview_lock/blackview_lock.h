...
// Forward declaration of our component class
class BlackviewLock;

// Global instance of our component to be accessible from C-style callbacks
static BlackviewLock *global_blackview_lock = nullptr;

// GAP event handler (for scanning)
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
  if (global_blackview_lock)
    global_blackview_lock->gap_event_handler(event, param);
}

// GATTC event handler (for connecting and data)
...
class BlackviewLock : public PollingComponent {
 public:
  ...
  BlackviewLock() : PollingComponent(60000) { global_blackview_lock = this; }
  ...
  void gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
    ...
    if (event == ESP_GATTC_WRITE_CHAR_EVT) {
      if (param->write.status == ESP_GATT_OK) {
        ...
        // Now that handshake is sent, we can register for notifications if needed
        // For now, we wait for the key.
      } else {
