...
// Blackview SE60 family (observed):
//   Service 0x1910
//   Write char   0x2B11 -> handle 0x0009
//   Notify char  0x2B10 -> handle 0x000B
//   CCCD                    handle 0x000C
class BlackviewLock : public Component, public ble_client::BLEClientNode {
 public:
  ...
  // ---- Back-compat no-ops so existing YAML/main.cpp compile unchanged ----
  ...
  void gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
    ...
    switch (event) {
      case ESP_GATTC_CONNECT_EVT: {
        ...
        if (have_bda_) {
          ...
          if (e == ESP_OK) {
            ...
            ESP_LOGD(TAG, "Requested encryption (MITM). Waiting ~%u ms before first HELLO.",
      ...
      case ESP_GATTC_WRITE_CHAR_EVT: {
        // Characteristic write completion (even for NO_RSP, IDF still signals here)
        ESP_LOGI(TAG, "Write OK (handle 0x%04X)", param->write.handle);
        break;
      }

      case ESP_GATTC_NOTIFY_EVT: {
        // Incoming notification/indication from the lock
        notify_counter_++;
  ...
  void loop() override {
    ...
    // Gate the very first HELLO on both CCCD-complete delay and encryption settle window.
    if (connected_flag_ && !hello_sent_after_cccd_ && post_cccd_hello_due_ms_) {
      const bool enc_window_done =
          (encryption_requested_ms_ == 0) || (now - encryption_requested_ms_ >= encryption_settle_ms_);
      if (enc_window_done && now >= post_cccd_hello_due_ms_) {
        post_cccd_hello_due_ms_ = 0;
        hello_sent_after_cccd_ = true;
        send_hello_(pending_gattc_if_, pending_conn_id_, /*prefer_write_no_rsp=*/true, "auto-v1");
      }
    }

    // Periodic keepalive HELLO (still gated by connection state)
    if (connected_flag_ && (wants_handshake_ || (now - last_hello_ms_ >= hello_interval_ms_))) {
      ...
      send_hello_(gi, conn_id, /*prefer_write_no_rsp=*/true, wants_handshake_ ? "manual" : "auto");
      wants_handshake_ = false;
    }
  }

 protected:
  // Fixed handles for this device family
  ...
  void send_hello_(esp_gatt_if_t gattc_if, uint16_t conn_id, bool prefer_write_no_rsp, const char *why) {
    ...
    // Alternate between the 16B (v1) and 18B (v0) frames to cover both observed variants.
    bool use_v1 = toggle_v1_;
    if (wants_handshake_) use_v1 = true;
    toggle_v1_ = !toggle_v1_;

    if (use_v1) {
      // 16-byte frame: A5 E5 00 0C | 8B nonce | 00 00 | CRC
      uint8_t buf[16];
      ...
      // 18-byte frame: A5 E5 00 0E | 00 00 | 8B nonce | 00 00 | CRC
      uint8_t buf[18];
  ...
  static constexpr uint32_t post_cccd_delay_ms_{150};  // short gap after CCCD
  uint16_t pending_conn_id_{0};
  esp_gatt_if_t pending_gattc_if_{0};

  // Encryption gating
  uint32_t encryption_requested_ms_{0};
  static constexpr uint32_t encryption_settle_ms_{1200};  // wait ~1.2s after requesting encryption

  // Handles (fixed for this device family)
